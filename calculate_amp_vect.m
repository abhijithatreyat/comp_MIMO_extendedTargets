...
function amp_vect = calculate_amp_vect(TX_pos, RX_pos, cart_x,...
    cart_y, cart_z, c, fc, N_points, t_ax ,array_x_m)
% Input:
...
amp_vect = zeros(length(t_ax), N_points);
    ...
    for target = 1:N_points
        ...
        d_T2P = sqrt((array_x_m-cart_x(target)).^2+ cart_y(target).^2 +...
            cart_z(target).^2); % 1 x N_tx 
        ...
        tau = (d_T2P+repmat(d_P2R,1,128))/c;
        ...
        amp_vect(:,target) = sum(pt_signal); % summing up signals from all point reflector to get the received signal in the antenna array   
    end
end
